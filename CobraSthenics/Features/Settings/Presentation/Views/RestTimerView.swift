...
public struct RestTimerSettingsView: View {
    ...
    public var body: some View {
        SettingsScreen(title: "Default Rest Timer", showsSaveCTA: true) {
            ringDisplay
                .padding(.top, AppSpacing.lg)
                .frame(maxWidth: .infinity, alignment: .center)

            SettingsListHeader("Presets")
            FilterChips(
            ...
            SettingsListHeader("Or set custom time")
            AppCard(padding: 14) {
                HStack {
            ...
            SettingsListHeader("Per-category overrides")
            SettingsListGroup {
                SettingsListRow(
                    systemImage: "square.stack.3d.up",
                    iconColor: AppColor.brand,
                    label: "Different rest per exercise type",
                    sub: "Override for strength, skill, conditioning",
                    showsDivider: false
                ) {
            ...
            if perCategory {
                VStack(spacing: AppSpacing.xs) {
                    overrideRow("Strength sets", value: "2:00", accent: AppColor.brand)
                    overrideRow("Skill holds", value: "3:00", accent: AppColor.green)
                    overrideRow("Conditioning", value: "1:00", accent: AppColor.red)
                }
                .padding(.top, AppSpacing.xs)
            }

            Text("Used as the default between all sets unless overridden per-exercise in the workout builder.")
                .font(.appBody)

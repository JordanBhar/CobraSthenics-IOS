...
public struct HelpFAQView: View {
    ...
    public var body: some View {
        SettingsScreen(title: "Help & FAQ") {
            SearchField(text: $searchText, placeholder: "Search help articles…")
                .padding(.top, AppSpacing.md)

            SettingsListHeader("Popular topics")
            ...
            SettingsListHeader("Contact")
            VStack(spacing: AppSpacing.xs) {
                contactCard(icon: "message.fill", color: AppColor.brand, title: "Chat with support", sub: "Live chat · usually responds in minutes")
                contactCard(icon: "envelope.fill", color: AppColor.textSecondary, title: "Email us", sub: "Usually responds within 24 hours")
            }
        }
    }
...
public struct SendFeedbackView: View {
    ...
    public var body: some View {
        SettingsScreen(title: "Send Feedback", showsSaveCTA: true, ctaTitle: "Send Feedback") {
            SettingsListHeader("Feedback type")
            FilterChips(
                options: ["Bug report", "Feature request", "General", "Design", "Performance"],
                selected: type,
                label: { $0 },
                onSelect: { type = $0 }
            )
            ...
            VStack(spacing: AppSpacing.sm) {
                FieldCard(label: "Subject", text: $subject)
                messageField
            }
            .padding(.top, AppSpacing.md)
            ...
            SettingsListGroup {
                SettingsListRow(
                    ...
                    label: "Attach current screenshot",
                    sub: "Helps the team see what you saw",
                    showsDivider: false
                ) {
            ...
            SettingsListHeader("Overall satisfaction · optional")
            AppCard(padding: 16) {
                HStack(spacing: 12) {
                    Spacer()
            ...
            Text("Your feedback goes directly to the Cobrasthenics team. We read every message.")
                .font(.appCaption)
                .foregroundStyle(AppColor.textSecondary)
                .padding(.top, AppSpacing.md)
        }
...
public struct DeleteAccountView: View {
    ...
    private var canDelete: Bool { typed == "DELETE" }
        ...
        ZStack(alignment: .bottom) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: AppSpacing.md) {
                    warningCard
                    SettingsListHeader("What will be deleted")
                    deletionList
                    SettingsListHeader("Before you go")
                    beforeYouGo
                    SettingsListHeader("Type DELETE to confirm")
                    confirmInput
                }
        ...
        AppCard(background: AppColor.red.opacity(0.08), border: AppColor.red.opacity(0.3)) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                Text("This cannot be undone")
                    ...
                    .foregroundStyle(AppColor.textPrimary)
                Text("Deleting your account permanently removes all your workouts, skill progress, PRs, and profile data. This action is irreversible.")
                    .font(.appBody)
                    .foregroundStyle(AppColor.textSecondary)
            }
        }
    ...
    private var beforeYouGo: some View {
        VStack(spacing: AppSpacing.xs) {
            beforeYouGoCard(icon: "square.and.arrow.down", title: "Export your data first", sub: "JSON, CSV or PDF — keep a record of your training")
            beforeYouGoCard(icon: "pause.circle", title: "Pause your account instead", sub: "Keep your data; come back any time")
        }
    }
    ...
    private var confirmInput: some View {
        VStack(spacing: AppSpacing.xs) {
            ...
            HStack(spacing: AppSpacing.xs) {
                smallChip("Clear", accent: false) { typed = "" }
                smallChip("Type DELETE", accent: true) { typed = "DELETE" }
                Spacer()
            }
        }
    }
    ...
    private var deleteCTA: some View {
        VStack(spacing: 12) {
            ...
            Button {
                ...
                Text("Changed your mind? Go back")
                    .font(.system(size: 12, weight: .medium))
                    .foregroundStyle(AppColor.textSecondary)
            }
            .buttonStyle(.plain)
